...
void SDMMeter::on_modbus_data(const std::vector<uint8_t> &data) {
  ...
  for (uint8_t i = 0; i < 3; i++) {
	...
	if(i==0) {
		ph_v = SDM_PHASE_1_VOLTAGE;
		ph_c = SDM_PHASE_1_CURRENT;
		ph_ap = SDM_PHASE_1_ACTIVE_POWER;
		ph_rp = SDM_PHASE_1_REACTIVE_POWER;
		ph_pf = SDM_PHASE_1_POWER_FACTOR;
	}
	if(i==1)  {
		ph_v = SDM_PHASE_2_VOLTAGE;
		ph_c = SDM_PHASE_2_CURRENT;
		ph_ap = SDM_PHASE_2_ACTIVE_POWER;
		ph_rp = SDM_PHASE_2_REACTIVE_POWER;
		ph_pf = SDM_PHASE_2_POWER_FACTOR;
	}
	if(i==2)  {
		ph_v = SDM_PHASE_3_VOLTAGE;
		ph_c = SDM_PHASE_3_CURRENT;
		ph_ap = SDM_PHASE_3_ACTIVE_POWER;
		ph_rp = SDM_PHASE_3_REACTIVE_POWER;
		ph_pf = SDM_PHASE_3_POWER_FACTOR;
	}
	 
    float voltage = sdm_meter_get_float(ph_v * 2 + (i * 4));
    float current = sdm_meter_get_float(ph_c * 2 + (i * 4));
    float active_power = sdm_meter_get_float(ph_ap * 2 + (i * 4));
    float apparent_power = sdm_meter_get_float(SDM_PHASE_1_APPARENT_POWER * 2 + (i * 4));
    float reactive_power = sdm_meter_get_float(ph_rp * 2 + (i * 4));
    float power_factor = sdm_meter_get_float(ph_pf * 2 + (i * 4));
